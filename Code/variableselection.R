...
#	= Modelos Clasificacion Supervisada: Probit =
#

rm(list = ls())

rutawork = '/home/jcmo/JCMO.Trabajo/Seminars,Visits&Talks/17-04.Cinvestav/17-04.Cinvestav_Taller/Code/'
rutadata = '/home/jcmo/JCMO.Trabajo/Seminars,Visits&Talks/17-04.Cinvestav/17-04.Cinvestav_Taller/Datos/'
rutaimages = '/home/jcmo/JCMO.Trabajo/Seminars,Visits&Talks/17-04.Cinvestav/17-04.Cinvestav_Taller/Images/'
...
#		Datos
#	----------------------------------------------------
dde <- read.csv(paste(rutadata,'variableselection_data.csv',sep = ""), header = TRUE, sep = ",", quote="\"", dec=".", fill = TRUE)

# 	Keys
# Y = 0/1 	# etiquetas de clasificación
# x = dde 	# variable predictora de interés
# z1-z5 	  # variables auxiliares
...
for(gt in 1:G){
  ...
  #	Output
  write(t(beta),file=paste(rutaimages,'variableselection_beta.out',sep = ""), ncol=7, append=T)
  print(c(gt,round(beta*100)/100))
}

#	Trace-plots para beta[1] y beta[2]
pdf(paste(rutaimages,'variableselection_beta_traces.pdf',sep = ""),width=7,height=5)
par(mfrow=c(2,1))
beta_out<- matrix(scan(paste(rutaimages,'variableselection_beta.out',sep = "")), ncol=7, byrow=T)
...
pdf(paste(rutaimages,'variableselection_beta_marginal_slope.pdf',sep = ""),width=7,height=5)
slp <- beta_out[101:1000,2]
par(mfrow=c(1,1))
par(mar=c(5,5,5,5))
plot(density(slp),type="l",xlab="DDE pendiente (beta_1)",ylab="Densidad Posterior",
     cex=1.2)
abline(v=mean(slp))
...
pdf(paste(rutaimages,'variableselection_beta_doseresponse.pdf',sep = ""),width=7,height=5)
par(mfrow=c(1,1))
par(mar=c(5,5,5,5))
...
plot(xtrue,post[,1],xlab="DDE",ylab="Pr(Etiqueta 1)",cex=1.2,
...
write.csv(t(table1),file=paste(rutaimages,'variableselection_beta_summary.csv',sep = ""))
...
for(gt in 1:G){
  ...
  #	Output
  write(t(beta),file=paste(rutaimages,'variableselection_beta_ss.out',sep = ""), ncol=7, append=T)
  print(c(gt,round(beta*100)/100))
}

#	Trace-plots para beta[1] y beta[2]
beta<- matrix(scan(paste(rutaimages,'variableselection_beta_ss.out',sep = "")), ncol=7, byrow=T)
# 2^p = Numero de posibles modelos
# En este caso, 2^p = 2^7 = 128

pdf(paste(rutaimages,'variableselection_beta_traces_ss.pdf',sep = ""),width=7,height=5)
...
pdf(paste(rutaimages,'variableselection_beta_doseresponse_ss.pdf',sep = ""),width=7,height=5)
par(mfrow=c(1,1))
par(mar=c(5,5,5,5))
xg<- seq(-2,2,length=100)  	# Grid
...
table1<- round(table1*100)/100
write(t(table1),file=paste(rutaimages,'variableselection_beta_summary_ss.csv',sep = ""),ncol=6)
...
write(t(table2),file=paste(rutaimages,'variableselection_beta_pmodel_ss.csv',sep = ""),ncol=8)

#
#	-- FIN: variableselection.r --